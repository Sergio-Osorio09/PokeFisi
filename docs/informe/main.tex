...
emparejamiento y lados alternados. La Tabla~\ref{tab:torneo} y la
Figura~\ref{fig:torneo} reportan el win-rate global de cada agente con su
intervalo de Wilson al 95\% (480 batallas por agente).
...
\bottomrule
\end{tabular}
\caption{Torneo de siete agentes (80 batallas por par, lados alternados):
win-rate global e intervalo de Wilson al 95\%. La Heurística Avanzada es la más
fuerte; el Genético-6 (seis pesos) queda por debajo del Genético-4 (cuatro), y su
enfrentamiento directo es un empate ($50$--$50$). Duración media: 11.7 turnos;
empates: 0\%.}
\label{tab:torneo}
\end{table}
...
independientes (Tabla~\ref{tab:panel}, Figura~\ref{fig:panel}).
...
\begin{tabular}{lcccc}
\toprule
Fitness vs.\ & Random & Básica & Avanzada & \textbf{Minimax-$d2$} \\
\midrule
1 rival (Básica) & 79.3 & 50.7 & 38.0 & \textbf{59.7} \\
Panel (R+B+A)    & 80.0 & 47.7 & 38.7 & \textbf{55.0} \\
\bottomrule
\end{tabular}}
\caption{Generalización del genético según su \emph{fitness} de entrenamiento:
win-rate medio (\%) sobre 5 semillas (60 batallas por rival y semilla). Contra el
rival \textbf{no visto} (Minimax-$d2$) el panel \emph{no} mejora ---de hecho rinde
algo peor y con más varianza ($55.0\pm8.2$ vs.\ $59.7\pm3.2$)--- y globalmente
ambos empatan ($55.3\%$ vs.\ $56.9\%$).}
\label{tab:panel}
\end{table}
...
$63.3\%$ contra la Básica, $82\%$ vs.\ $87.3\%$ contra el azar y un empate técnico
en enfrentamiento directo ($45.3\%$), a un coste $\sim 3\times$ por decisión
(Tabla~\ref{tab:avzmej}). Y \emph{dentro} del minimax con genético tampoco: el
Genético-6 no supera al Genético-4 (Experimento~A, Tabla~\ref{tab:torneo}). De
hecho, el propio algoritmo genético \textbf{anula} el componente \emph{peligro de
ser noqueado} (peso evolucionado $0.002$): la evolución ``descubre'' que ese
componente \emph{duplica} una señal ---la táctica de un turno--- que el minimax
\emph{ya} resuelve al simular el intercambio. El cuello de botella es, por tanto,
la \emph{simplicidad del dominio} (daño dominado por el HP, sin estados de
alteración), no la evaluación ni el optimizador: una buena función de evaluación
debe \emph{complementar} la búsqueda, no \emph{duplicar} la táctica que el árbol ya
calcula.
...
\midrule
H.\ Avanzada (4) & 63.3 & 87.3 & ---  & \textbf{0.12} \\
H.\ Mejorada (6) & 58.0 & 82.0 & 45.3 & 0.40 \\
\bottomrule
\end{tabular}}
\caption{Heurística Avanzada (4) vs.\ Mejorada (6) a un solo nivel ($150$ batallas,
mismos escenarios). La evaluación más rica no mejora el win-rate de forma
apreciable (enfrentamiento directo $45.3\%$, dentro del ruido) y cuesta
$\sim 3\times$ por decisión.}
\label{tab:avzmej}
\end{table}

\section{Resultados y Discusión}

\paragraph{Jerarquía de agentes.} El torneo (Tabla~\ref{tab:torneo},
Figura~\ref{fig:torneo}) sitúa al agente aleatorio como el más débil con claridad
...
contra uno solo (Experimento~D, Tabla~\ref{tab:panel}) ---un resultado nulo que
...
componentes lo superan ---esta última ni a un nivel (Tabla~\ref{tab:avzmej}) ni
dentro del minimax con genético (Genético-6). El cuello de botella no es, por
...
(Tabla~\ref{tab:torneo}) y la propia evolución llegó a anular los componentes
