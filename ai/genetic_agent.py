...
GeneticImprovedAgent es la variante sobre la evaluación MEJORADA (6 pesos).
"""
import json
from ai.minimax_agent import MinimaxAgent
from ai.minimax_improved_agent import MinimaxImprovedAgent, DEFAULT_WEIGHTS_6
...
class GeneticImprovedAgent(MinimaxImprovedAgent):
    """
    MinimaxImprovedAgent (evaluación de 6 componentes de la H. Mejorada) con
    los pesos evolucionados por Algoritmo Genético. La heurística de las hojas
    es la Mejorada; el AG decide CUÁNTO pesa cada uno de sus 6 componentes.
    """
    ...
    def __init__(self, weights: list[float], metadata: dict | None = None):
        ...
        self.name              = f"Genetico-Mej g={self._gens} d={self._depth}"
        self.battles_trained   = self._gens
        self.win_rate_training = self._fit

    def get_info_lines(self) -> list[str]:
        labels = ["superv", "hp_pond", "ko+", "ko-", "cob", "vel"]
        w      = self.weights
        w_str  = "  ".join(f"{l}:{w[i]:.2f}" for i, l in enumerate(labels))
        return [
            f"Minimax-Mejorada (d={self._depth}) con 6 pesos evolucionados por AG.",
            f"Generaciones: {self._gens}  Poblacion: {self._pop}"
            f"  Fitness: {self._fit:.0%}",
            f"Pesos: {w_str}",
        ]
    ...
    def choose_action(self, state, player_id):
        ...
        if self.last_brain_data is not None:
            labels = ["superv", "hp_pond", "ko+", "ko-", "cob", "vel"]
            base   = DEFAULT_WEIGHTS_6
            self.last_brain_data["formula"] = (
                f"minimax-mejorada evolucionado (g={self._gens}, fitness {self._fit:.0%})")
            self.last_brain_data["weights"] = [
                (labels[i], self.weights[i], base[i]) for i in range(len(labels))
            ]
        return action
...
def load_genetic_agent(path: str) -> GeneticAgent:
    with open(path, encoding="utf-8") as f:
        data = json.load(f)
    if data.get("n_weights", len(data["weights"])) == 6:
        return GeneticImprovedAgent(weights=data["weights"], metadata=data)
